...
function buildFlowData(questions: Question[]) {
  ...
  for (const [, dimQuestions] of Object.entries(dimGroups)) {
    ...
    // 构建父子关系：子题.based_on_record_index == 父题.question_id
    const nodeChildren = new Map<string, Question[]>()
    dimQuestions.forEach(q => {
      if (q.based_on_record_index !== '-1') {
        const parentQid = q.based_on_record_index
        const parent = dimQuestions.find(p => p.question_id === parentQid)
        if (parent) {
    ...
    // 计算子树宽度（叶节点 = NODE_W，内部节点 = max(NODE_W, sum(children) + gaps)）
    ...
    dimQuestions.forEach(q => {
      if (q.based_on_record_index !== '-1') {
        const parent = dimQuestions.find(p => p.question_id === q.based_on_record_index)
        if (parent) assigned.add(q.question_id)
      }
    })
    const roots = dimQuestions.filter(q => !assigned.has(q.question_id))

    // 计算所有根子树宽度
    roots.forEach(r => calcWidth(r.question_id))

    // 根节点横向排列，每棵子树按实际宽度占位，子树间留 H_GAP
    let totalRootWidth = 0
    roots.forEach((r, i) => {
      totalRootWidth += subtreeWidth.get(r.question_id)!
      if (i > 0) totalRootWidth += H_GAP
    })

    let curX = -totalRootWidth / 2
    roots.forEach((r, i) => {
...
export default function ExamHistory() {
  ...
  const handleSelectRecord = async (record: ExamRecord) => {
    ...
    try {
      ...
      if (!examItemId) {
        setQuestionsError('缺少考试项信息，无法加载题目')
        return
      }
      const res = await fetch(`${API_BASE}/exam_items/${examItemId}/questions`, { credentials: 'include' })
      if (!res.ok) {
        setQuestionsError(`加载题目失败 (HTTP ${res.status})`)
        return
      }
      const json = await res.json()
      const recs = Array.isArray(json) ? json : json.data ?? []
      const match = recs.find((r: { exam_id?: string }) => r.exam_id === record.exam_id)
      const qs = match?.questions ?? (recs.length > 0 && recs[0].questions ? recs[0].questions : null)
      if (qs && qs.length > 0) {
        setQuestions(qs)
      } else {
        setQuestionsError('该考试记录暂无题目数据')
      }
    } catch (err) {
      setQuestionsError(err instanceof Error ? err.message : '加载题目异常')
    } finally {
  ...
  return (
    <div className="history-page">
      ...
      {selectedRecord && !questionsLoading && questionsError && (
        <div className="qflow-overlay" onClick={handleCloseModal}>
          <div className="qflow-modal" onClick={e => e.stopPropagation()}>
            <div className="qflow-header">
              <h3>加载失败</h3>
            ...
            <div className="history-error" style={{ margin: '24px' }}>
              <i className="fas fa-exclamation-circle"></i>
              <span>{questionsError}</span>
              <button onClick={() => handleSelectRecord(selectedRecord)}>重试</button>
            </div>
          </div>
        </div>
      )}
      {selectedRecord && !questionsLoading && !questionsError && questions.length > 0 && (
        <QuestionFlowModal
          questions={questions}
