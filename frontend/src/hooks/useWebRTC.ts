...
export function useWebRTC(showToast: ShowToastFn) {
  ...
  const [remoteAudioStarted, setRemoteAudioStarted] = useState(false)
  const [audioDetected, setAudioDetected] = useState(false)
  const audioDetectedFlagRef = useRef(false)
  ...
  const stopMonitor = useCallback(() => {
    ...
    audioDetectedFlagRef.current = false
  }, [])
  ...
  const startMonitor = useCallback(async (stream: MediaStream) => {
    ...
    processor.onaudioprocess = (e) => {
      ...
      if (!audioDetectedFlagRef.current) {
        audioDetectedFlagRef.current = true
        setAudioDetected(true)
        console.log('[Monitor] 检测到有效音频输入')
      }
  ...
  const startSession = useCallback(async (micStream: MediaStream | null, examInfo?: Record<string, unknown>) => {
    setStarting(true)
    cleanupConnection()
    ...
    try {
      ...
      const offerRes = await fetch(offerUrl, {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ sdp: pc.localDescription!.sdp, type: 'offer', pc_id: null, restart_pc: false, exam_info: examInfo ?? {} }),
      })
  ...
  return { sessionId, chatStatus, starting, remoteAudioStarted, audioDetected, startSession, closeSession, sendDCMessage, onDCMessage, replaceAudioTrack, cleanupConnection }
}
