...
from AIOralExamSystem.Agent.Judger import JudgerAgent, MainJudgerAgent
...
class LLMService(FrameProcessor):
    def __init__(self, monitor: GlobalMonitor, current_user: dict, history: List[Dict[str, str]] = []):
        super().__init__()
  ...
  <p style="margin:0;font-size:13px;color:#666;line-height:1.6;">评价缘由：{reson}</p>
    ...
    #             await self.push_frame(frame, direction)
    #         else:
    #             self.add_user_msg(messages[0]['content'])
    ...
    #                                 print(e)
    #             print(''.join(Output))
    #             await self.push_frame(LLMTextFrame(''.join(Output)))
