...
def _setup_webrtc_routes(app: FastAPI, args: argparse.Namespace):
    ...
    @app.post("/api/offer")
    async def offer(request: SmallWebRTCRequest, exam_info: dict, background_tasks: BackgroundTasks, current_user: dict = Depends(get_current_user)):
        ...
        async def webrtc_connection_callback(connection: SmallWebRTCConnection):
            ...
            runner_args.cli_args = args
            background_tasks.add_task(bot_module.bot, runner_args, exam_info, current_user)
    ...
    @app.patch("/api/offer")
    async def ice_candidate(request: SmallWebRTCPatchRequest, current_user: dict = Depends(get_current_user)):
    ...
    @app.post("/start")
    async def rtvi_start(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    async def proxy_request(
        session_id: str, path: str, request: Request, background_tasks: BackgroundTasks, current_user: dict = Depends(get_current_user)
    ):
        ...
        if path.endswith("api/offer"):
            ...
            try:
                ...
                if request.method == HTTPMethod.POST.value:
                    ...
                    return await offer(webrtc_request, request_data['exam_info'], background_tasks, current_user)
                elif request.method == HTTPMethod.PATCH.value:
                    patch_request = SmallWebRTCPatchRequest(
                        pc_id=request_data["pc_id"],
                        candidates=[IceCandidate(**c) for c in request_data.get("candidates", [])],
                    )
                    return await ice_candidate(patch_request, current_user)
            except Exception as e:
